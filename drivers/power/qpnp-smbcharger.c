...
struct smbchg_chip {
	...
	struct delayed_work		update_period_work;
...
#define HOT_DESIGN_MAX_CURRENT 1500   //2000-->1500
...
static int smbchg_default_dcp_icl_ma = 1000;  //1500 -->1000
module_param_named(
...
#define BATT_HEALTH_TEMP_HOT	(550)
#define BATT_HEALTH_TEMP_WARM	(450)
#define BATT_HEALTH_TEMP_COOL	(100)
#define BATT_HEALTH_TEMP_COLD	(0)
...
#define IINLIM_2000		1500   //2000->1500
...
static void smbchg_external_power_changed(struct power_supply *psy)
{
	...
	if (current_limit != chip->usb_target_current_ma) {
		...
		if (current_limit <= IINLIM_2000) {
			...
			if((strcmp(usb_type_name,"OTHER")==0)&&(current_limit >= 1000))
			{
				current_limit =  1000;
				limit_usbin_max = 1000;
				pr_smb(PR_STATUS, "usb type = %s,Diego limit current_limit = %d\n",usb_type_name,current_limit);
			}

			if((limit_usbin_max==1000)&&(current_limit>=1000)){
				current_limit = 1000;
				pr_info("fake insert/remove. current_limit  to %d\n", current_limit);
			}
...
#define DEFAULT_CDP_MA		1000   //1500 -->1000
...
#define INSERT_REMOVE_DELTA_MS   1500
#define PROTECT_DELTA_MS    50
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	int rc;
	char *usb_type_name = "null";
	static unsigned long  cat_fake_insert_jiffies = 0;
	unsigned long delta = 0;

	pr_smb(PR_STATUS, "triggered\n");

	if (cat_fake_insert_jiffies == 0) {
		cat_fake_insert_jiffies = jiffies;
		pr_info("start check at %ld\n", cat_fake_insert_jiffies);
	} else {
		delta = jiffies - cat_fake_insert_jiffies;
		pr_info("%ld=%ld-%ld\n",
			delta, jiffies, cat_fake_insert_jiffies);
		if ((!time_after(jiffies,cat_fake_insert_jiffies + INSERT_REMOVE_DELTA_MS ))&&
			(time_after(jiffies,cat_fake_insert_jiffies+PROTECT_DELTA_MS ))){
			limit_usbin_max =1000;
		}
		else{
			cat_fake_insert_jiffies=0;
			limit_usbin_max =0;
		}
	}
	/* usb inserted */
...
static enum power_supply_property smbchg_battery_properties[] = {
	...
	POWER_SUPPLY_PROP_STOPCHARGING,
	POWER_SUPPLY_PROP_STARTCHARGING,
};
...
static bool ffbm_mode=false;
static int __init early_parse_ffbm_mode(char *cmdline)
{
...
early_param("androidboot.mode", early_parse_ffbm_mode);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ALLOW_HVDCP3:
		...
		break;
	case POWER_SUPPLY_PROP_STOPCHARGING:
		val->intval = 0;
		if (ffbm_mode) {
			smbchg_charging_en(chip, false);
			pr_info("__POWER_SUPPLY_PROP_STOPCHARGING__ 0  ffbm_mode is %d\n", ffbm_mode);
		} else {
			pr_info("__POWER_SUPPLY_PROP_STOPCHARGING__   0\n");
		...
		break;
	case POWER_SUPPLY_PROP_STARTCHARGING:
		val->intval = 1;
		if(ffbm_mode){
			smbchg_charging_en(chip,true);
			pr_info("__POWER_SUPPLY_PROP_STARTCHARGING__ 1 ffbm_mode is %d\n", ffbm_mode);
		} else {
			pr_info("__POWER_SUPPLY_PROP_STARTCHARGING__   1\n");
			vote(chip->usb_suspend_votable, USER_EN_VOTER,
					!val->intval, 0);
			vote(chip->dc_suspend_votable, USER_EN_VOTER,
					!val->intval, 0);
...
#define DEFAULT_FCC_MA			1500  //2000->1500
...
static int set_period_ms(const char *val, struct kernel_param *kp)
{
	...
	if (global_chip) {
		...
		cancel_delayed_work_sync(&global_chip->update_period_work);
		schedule_delayed_work(&global_chip->update_period_work,
...
static void update_period(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct smbchg_chip *chip = container_of(dwork,
				struct smbchg_chip, update_period_work);

	int period = 500;
	int temp,voltage,cap,status,charge_type,present,chg_current,usb_present,usb_current,usb_vol,batt_healthy;
	...
	if (usb_psy)
		usb_psy->get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_VOLTAGE_NOW, &prop);
	if(smbchg_debug_mask == 0xFF)   //used smbchg_debug_mask for log debug.
		dump_regs(chip);
	...
	usb_vol		= 	prop.intval;
	batt_healthy    =	get_prop_batt_health(chip);
	printk_counter++;
	/*if period_ms is bigger than 500ms,must output the logs directly.*/
	...
	|| (old_present != present) || (old_usb_present != usb_present )||(printk_counter>=5))
	{
		pr_info("***temp=%d,vol=%d,cap=%d,status=%d,chg_state=%d,current=%d,present=%d,usb_present=%d\n,chip->chg_enabled=%d\n",
			temp,voltage,cap,status,charge_type,chg_current,present,usb_present,chip->chg_enabled);
		pr_info("***present=%d,usb_present=%d,usb_current=%d,usb_vol=%d,healthy=%d(1good,2hot,6cold,9warm)\n",
                            present,usb_present,usb_current,usb_vol,batt_healthy);
		old_temp = temp;
		old_cap = cap;
		old_status = status;
		old_present = present;
	...
	if(temp >= 60)
	{
		pr_info("Temp protect. kernel_power_off at temp :%d\n",temp);
		kernel_power_off();
	}
	...
	if (period_ms >= 500) {
		...
		if( (cap <= 5)||(temp >= 57)  ) //PM modify for LOW battery or High TEMP
			period = LOW_SOC_HEARTBEAT_MS;
		else
			period = HEARTBEAT_MS;
	}

	schedule_delayed_work(&chip->update_period_work,
				      round_jiffies_relative(msecs_to_jiffies
							     (period)));
}
...
static int qpnp_smb_set_in_thermal(int val)
{
	...
	pr_smb(PR_MISC, "current_limit usb max= %d,hot_design_current=%d,val=%d\n", current_limit,hot_design_current,val);
	if((limit_usbin_max==1000)&&(current_limit>=1000)){
		current_limit =1000;
		pr_smb(PR_MISC, "diego current_limit limit to= %d\n", current_limit);
	}
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->update_period_work, update_period);
	...
	schedule_delayed_work(&chip->update_period_work,
				  round_jiffies_relative(msecs_to_jiffies(period_ms)));
