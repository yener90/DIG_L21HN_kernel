...
static int fg_sram_update_period_ms_ffbm = 5000;
...
resched:
	if (battid_valid) {
		...
		if(ffbm_mode){
			pr_info("boot mode ffbm_mode=%d\n",ffbm_mode);
			*resched_ms =fg_sram_update_period_ms_ffbm;
			}
		else{
...
wait:
	...
	pr_info("Diego fg_batt_type:%s\n",chip->batt_type);
	app_info_set("battery_id", chip->batt_type);
