...
extern void enable_irq_for_extern_usage(bool enable);
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		...
		if (!pinfo->cont_splash_enabled) {
			...
			if (synap_gesture_onoff() && strstr(pdata->panel_info.panel_name, "TD4100") != NULL){
				enable_irq_for_extern_usage(false);
			}
			...
			if (synap_gesture_onoff() && strstr(pdata->panel_info.panel_name, "TD4100") != NULL){
				enable_irq_for_extern_usage(true);
			}
...
int panel_check_live_status(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	int j = 0;
	int i = 0;
	int k = ctrl->panel_data.panel_info.read_byte;//the value is esd-check-read-byte in lcm dtsi
	/* success on retrun 1,otherwise return 0 or -1 */
	int ret = 1;
	int compare_reg = 0;
	#define MAX_RETURN_COUNT_ESD 4
	#define REPEAT_COUNT 5
	char esd_buf[MAX_RETURN_COUNT_ESD]={0xff, 0xff, 0xff, 0xff};
	...
	for(j = 0;j < ctrl->esd_cmds.cmd_cnt;j++)
	{
		...
		do
		{
			...
			if(strstr(ctrl->panel_data.panel_info.panel_name, "TD4100") != NULL && ctrl->esd_cmds.cmds[j].payload[0]==0xFA)
			{
					for(i=k;i<(ctrl->esd_cmds.cmds[j].dchdr.dlen-1);i++)
					{
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	rc = of_property_read_u32(np, "qcom,mdss-dsi-bl-half-level", &tmp);
	pinfo->bl_half =(!rc ? tmp : 1620);
	rc = of_property_read_u32(np, "qcom,mdss-dsi-bl-target-level", &tmp);
	pinfo->bl_target =(!rc ? tmp : 1620);
	ctrl_pdata->bklt_max = pinfo->bl_max;

	rc = of_property_read_u32(np, "qcom,esd-check-read-byte", &tmp);//read esd-check-read-byte from lcm dtsi
	pinfo->read_byte =(!rc ? tmp : 0);
...
//Add,@wangdongbo.wt_coompitable_BOE_lcd_panel_B3_B8
#define LCM_NAME_BOE_B3_OLD			"TD4100_720p_video_BOE"
#define LCM_NAME_BOE_B3_NEW		"TD4100_B3_720p_video_BOE"
#define LCM_NAME_BOE_B8				"TD4100_B8_720p_video_BOE"
static char Lcm_panel_name[MDSS_MAX_PANEL_LEN]={0};

int get_mdss_dsi_panel_name(void)
{
	 char *ptr=NULL;

	 ptr = strstr(Lcm_panel_name, LCM_NAME_BOE_B3_OLD);
	 if(ptr != NULL)
	 {
		pr_err("mdss_dsi_panel: Panel -BOE_B3_OLD \n");
		return 1;
	 }

	 ptr = strstr(Lcm_panel_name, LCM_NAME_BOE_B3_NEW);
	 if(ptr != NULL)
	 {
		pr_err("mdss_dsi_panel: Panel -BOE_B3_NEW \n");
		return 2;
	 }

	ptr = strstr(Lcm_panel_name, LCM_NAME_BOE_B8);
	 if(ptr != NULL)
	 {
		pr_err("mdss_dsi_panel: Panel -BOE_B8 \n");
		return 3;
	 }
	return 0;
}

EXPORT_SYMBOL(get_mdss_dsi_panel_name);
//Add,@wangdongbo.wt_coompitable_BOE_lcd_panel_B3_B8
	...
	int ndx)
{
	...
	if (!panel_name) {
		...
		strlcpy(Lcm_panel_name, panel_name, MDSS_MAX_PANEL_LEN);//Add,@wangdongbo.wt_coompitable_BOE_lcd_panel_B3_B8
	}
#ifdef CONFIG_HUAWEI_KERNEL_LCD
